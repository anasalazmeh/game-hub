...
const GameGrid = () => {
    ...
    if (error)
        return <Text color="red" fontSize='50px'>{error.message}</Text>;
    return (<>
            <InfiniteScroll dataLength={fetchScroll} hasMore={!!hasNextPage} next={() => fetchNextPage()} loader={<HStack justifyContent='center'>
        ...
        <GameGardContainer key={item.id}><GameCard game={item}  ></GameCard></GameGardContainer>
                    ...
                    {data?.pages.map((data, index) => <React.Fragment key={index}>
                            {data.results.map(item => <GameGardContainer key={item.id}><GameCard game={item}></GameCard></GameGardContainer>)}
                        </React.Fragment>)}
                </SimpleGrid>
            </InfiniteScroll>
